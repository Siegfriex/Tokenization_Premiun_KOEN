...
>
> Director operating decision of record: **R0 `APPROVE_BLOCK_LEVEL_PRIMARY_INTERPRETATION`**,
> with the `SM-01`, `M3-01`, `ID-03`, `ID-04`, `SPEC-01`/`SPEC-02` dispositions transcribed in §8.
...
because its option tables were superseded by the Director ruling transcribed in §8 and merging a
...
asymmetry once byte representation volume is separated. The two are reported separately and never
...
`G5_REALIZED_MODEL_CONTRACT_v001.json` on `main`. The Director ruling forbids any post-hoc change to
...
**No random intercept on `source` is estimated, and this is not a choice.** SSOT §18.1/§18.2 write
`u_source[i]`; SSOT §20.1 supersedes it whenever source levels are few or a source is nearly fully
coupled with a domain. The realized cohort has **two** sources, `other` is the only domain occurring
under both, and G5 recorded `SOURCE_DOMAIN_IDENTIFIABILITY = COMPOSITE_CELL_CONTROL_ONLY`. A
two-level random intercept has no estimable variance component. The `u_source` term is therefore
realized as the **fixed** `source_domain_cell`, exactly as §20.1's second clause directs.

This also closes SSOT §24 item 9 ("source fixed vs random specification, *within the feasible
range*"): the feasible range contains only the fixed specification. That is a stated limitation, not
an omission.
...
COMPANION       pair-level multiplier (wild) bootstrap, Rademacher weights
                B = 2000 · seed derived from the protocol identifier, recorded before execution
...
### 3.3 Precision at this sample size
...
marginal / conditional R²          → not applicable: no random effect is estimable (§3.1)
...
At `N = 3,835,988` every nested likelihood-ratio test is expected to reject, including for blocks of
negligible explanatory size. An LR test result may therefore **never** be reported as the finding.
It is reported beside `ΔR²` and partial `R²`, and any block whose partial `R²` is negligible must be
described as negligible **regardless of its test statistic** (SSOT §17.3, §29).
...
|---|---|---|
| `ko/en_script_type_count`, `ko/en_script_switch_count` | **no `INDEPENDENT_SUBSTANTIVE_EFFECT`.** Only the block-level contribution of the script-mixing construct is interpreted. | `SM-01`; Director ruling §8 |
| `ko_chunk_count_log`, `en_chunk_count_log` and the D-05 chunk-scale terms | **no substantive interpretation of raw chunk-count coefficients.** Every M3 table carries the coefficient-level instability warning of §6.5. | `M3-01`; Director ruling §8 |
...
random intercept on source          not estimable at 2 levels (§3.1)
```

### 7.5 The three boundaries
...
## 8. Director dispositions of record

Transcribed from the operating decision, binding on this protocol.
...
| 10 | `SM-01` alternative parameterization: `1{script_type_count ≥ 2}` + `script_switch_count`, or a single representative | Director ruling §8 |
| 11 | `M3-01` chunk-density reparameterization — **rank-check first per §8.2** | Director ruling §8 |
...
5. `ENG-OBS-001` telemetry on any stage exceeding 30 seconds — 10-second periodic samples carrying
   ...
   status, with no fabricated percentage or ETA.
